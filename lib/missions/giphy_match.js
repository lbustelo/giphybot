...
challenge.prototype.onPost = function(post, reply){
  console.log(`giphy_match: for game ${this.game.id}...`);
  store.withinTransaction(function(){
    return getGiphyToMatch(this.game).then(
          ...
          if(cheating){
            ...
            return Promise.all([
              ...
              function([points, bonus, isAutoRestart]){
                console.log(`giphy_match: points awarded`);
                points.inc(bonus);
...
challenge.prototype._pickGiphy = function(){
  return store.Giphy.randomPick().then(
    function(pickedGiphy){
      if(pickedGiphy){
        console.log(`giphy_match: new giphy is ${pickedGiphy.id}`);
